...
# Feature normalization using the Min-Max Scalar appraoch
def get_minmax_normalized_X_train_test(X_train, X_test):
    minmaxscaler = MinMaxScaler()
    X_train_norm = minmaxscaler.fit_transform(X_train)
    X_test_norm = minmaxscaler.transform(X_test)
    return X_train_norm, X_test_norm

# Feature normalization using the Standard Scalar appraoch
def get_standard_normalized_X_train_test(X_train, X_test):
    standartscaler = StandardScaler()
    X_train_norm = standartscaler.fit_transform(X_train)
    X_test_norm = standartscaler.transform(X_test)
    return X_train_norm, X_test_norm
...
def dataset_imbalance_test(target_output, dataset_text):
    ...
    plt.bar(classes, class_distribution, color=('springgreen', 'fuchsia', 'violet', 'pink'))
    ...
    plt.ylabel("Total Number of Samples of each Class", fontweight="bold", fontsize=15)
    plt.xticks(rotation=30, ha='right', fontsize=15, fontweight="bold")
    plt.yticks(fontsize=13, fontweight="bold")
    # plt.pie(class_distribution)
    plt.savefig(path_to_save_plots + "classification_imbalance_test.png", dpi=300)
    plt.show()
...
def display_feature_importance_coefficients(importance, feature_names):
    ...
    plt.savefig(path_to_save_plots + "feature_importance_coefficients.png", dpi = 300)
    plt.show()        
...
def feature_selection_with_pearson_corr_coef(df, target_ouput_name, corr):
    ...
    # print(f"df from the corr. coef. part: {df}")
...
def new_df_create(df):
    new_df = pd.DataFrame(columns=df_cols)
    # print(f"\nnew_df:{new_df}\n")
    df_labels = []
    df_items_ids = []
    for dfID in range(len(df)):
        labelName = df['labels'].iloc[dfID]
        if labelName not in df_labels:
            df_labels.append(labelName)
            df_items_ids.append([dfID])
            # print(dfID)
        else:
            df_labels_id = df_labels.index(labelName)
            df_items_ids[df_labels_id].append(dfID)

    for dfl_i in range(len(df_labels)):
        print(f"df label:{df_labels[dfl_i]}\n Number of samples: {len(df_items_ids[dfl_i])}\n")
        
        df_new_items_ids = []
        if df_labels[dfl_i] == "Normal":
            df_new_items_ids = df_items_ids[dfl_i]
        else:
            for df_item_id in range(len(df_items_ids[dfl_i])):            
                if df_labels[dfl_i] == "gafgyt":
                    if df_item_id % 4 == 0:
                        df_new_items_ids.append(df_items_ids[dfl_i][df_item_id])
                
                if df_labels[dfl_i] == "mirai":
                    if df_item_id % 7 == 0:
                        df_new_items_ids.append(df_items_ids[dfl_i][df_item_id])
     
    for new_item_id in range( 3 * len(df_items_ids[0]) ):
        print(f"new_item_id:{new_item_id}")        
        if new_item_id % 3 == 0:
            temp_df_row = df.iloc[df_items_ids[0][i]]
            new_df = new_df.append(temp_df_row, ignore_index=True)
            i +=1
        
        if new_item_id % 3 == 1:
            temp_df_row = df.iloc[df_items_ids[1][j]]
            new_df = new_df.append(temp_df_row, ignore_index=True)
            j +=1
        
        if new_item_id % 3 == 2:
            temp_df_row = df.iloc[df_items_ids[2][k]]
            new_df = new_df.append(temp_df_row, ignore_index=True)
            k +=1
    print(f"\nNew df: \n{new_df.head(20)}\n")

            # new_df.append(df.iloc[df_items_ids[0][]], ignore_index=True)

    new_df.to_csv("new_df.csv")
    print("New dataframe written successfully!")

global_test_size = .4113
# .357
global_random_state_number = 101
...
if __name__ == "__main__":
    ...
    # df = df.iloc[:1000]
    dataset_text = "Dataset Class Distribution Imbalance Test for the N-BaIoT Dataset"
    ...
    # df["attack"] = Y_label_encoder.fit_transform(df["attack"]).astype('float64')

    # x_train, x_test, y_train, y_test = train_test_split(df[df.columns[2:5]].values, df['attack'].values, \
    #                                     test_size = global_test_size, random_state = global_random_state_number)

    # dataset_text = "Y Train Set"
    # dataset_imbalance_test(target_output=pd.DataFrame(y_train, columns=['Train']), dataset_text=dataset_text)
    # dataset_text = "Y Test Set"
    # dataset_imbalance_test(target_output=pd.DataFrame(y_test, columns=['Test']), dataset_text=dataset_text)
    ...
    # dataset_text = "Dataset Class Distribution Imbalance Test for \nthe KDD99 Dataset after Dropping Duplicate Records"
    # dataset_imbalance_test(target_output=target_output[target_names], dataset_text=dataset_text) 

    # # Dataset target output class distribution imbalance test
    # dataset_text = "Dataset Sub-Class Distribution Imbalance Test for \nthe KDD99 Dataset after Dropping Duplicate Records"
    # dataset_imbalance_test(target_output=df['labels'], dataset_text=dataset_text) 
    ##------------------------------------------------------------------------------------------
    
    # Drop unuasable features of the dataset
    df = df.drop(["labels", 'sublabels'], axis = 1) 
    ...
    for df_col in df.columns:
        ...
        if isinstance(df[df_col].iloc[0], str):  
            ...
            df[df_col] = label_encoder_df_col.fit_transform(df[df_col]).astype('int64')
    # print(f"The dataframe after the label encoding process: \n{df}\n")
    ...
    # sfm_for_backward = SelectFromModel(estimator = fs_model, threshold=threshold).fit(X, y)
    # bw_features = X.columns[sfm_for_backward.get_support()]
    # print(bw_features)
    # Sequential feature selection via the backward feature selection approach
    backward_selected_features = backward_sequential_feature_selection(#X=X[bw_features], 
                                                                       X=X,
                                                                       y=y, 
                                                                       feature_select_model = fs_model, 
                                                                       number_of_features_to_select=number_of_features_to_select
